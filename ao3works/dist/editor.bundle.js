(() => {
  ...
  var setContent = (content, emitUpdate = false, parseOptions = {}, options = {}) => ({ editor, tr: tr2, dispatch, commands: commands2 }) => {
    ...
    if (parseOptions.preserveWhitespace !== "full") {
      ...
      if (dispatch) {
        tr2.replaceWith(0, doc3.content.size, document3).setMeta("preventUpdate", !emitUpdate);
      }
      return true;
    }
    if (dispatch) {
...
.tippy-box[data-animation=fade][data-state=hidden] {
  ...
  // node_modules/zeed-dom/node_modules/entities/dist/esm/generated/decode-data-html.js
  ...
  // node_modules/zeed-dom/node_modules/entities/dist/esm/generated/decode-data-xml.js
  var decode_data_xml_default = new Uint16Array(
    // prettier-ignore
    "\u0200aglq	\x1B\u026D\0\0p;\u4026os;\u4027t;\u403Et;\u403Cuot;\u4022".split("").map((c) => c.charCodeAt(0))
  );

  // node_modules/zeed-dom/node_modules/entities/dist/esm/decode-codepoint.js
  var _a;
  ...
  // node_modules/zeed-dom/node_modules/entities/dist/esm/decode.js
  var CharCodes;
  (function(CharCodes2) {
  ...
  // node_modules/zeed-dom/node_modules/entities/dist/esm/generated/encode-html.js
  ...
  // node_modules/zeed-dom/node_modules/entities/dist/esm/escape.js
  ...
  // node_modules/zeed-dom/node_modules/entities/dist/esm/index.js
  var EntityLevel;
  ...
  function matchSelector(selector, element, { debug = false } = {}) {
    for (const rules of parseSelector2(selector)) {
      if (debug) {
    ...
    const parser = new HtmlParser({
      ...
      scanner: {
        startElement(tagName, attrs, isSelfClosing) {
          ...
          const parentNode2 = stack[stack.length - 1];
          if (parentNode2) {
        ...
        characters(text) {
          ...
          const parentNode2 = stack[stack.length - 1];
          if (parentNode2?.lastChild?.nodeType === VNode.TEXT_NODE) {
  ...
  function getHTMLFromFragment2(doc3, schema, options) {
...
3. "-" cannot repeat`);
  ...
  function skinInnerHtmlFromAttrs(type, contentData) {
    const data = contentData || {};
    const w = typeof window !== "undefined" ? window : {};
    switch (type) {
      case "imessage":
        return w.buildIosPhoneHTML ? w.buildIosPhoneHTML(data) : "";
      case "tweet":
        return w.buildTweetHTML ? w.buildTweetHTML(data) : "";
      case "letter":
        return w.buildLetterHTML ? w.buildLetterHTML(data) : "";
      case "spoiler":
        return w.buildSpoilerHTML ? w.buildSpoilerHTML(data) : "";
      case "chatroom":
        return w.buildSlackChatHTML ? w.buildSlackChatHTML(data) : "";
      case "whatsapp":
        return w.buildWhatsAppHTML ? w.buildWhatsAppHTML(data) : "";
      case "gmail":
        return w.buildGmailHTML ? w.buildGmailHTML(data) : "";
      case "snapchat":
        return w.buildSnapchatHTML ? w.buildSnapchatHTML(data) : "";
      case "tumblr":
        return w.buildTumblrHTML ? w.buildTumblrHTML(data) : "";
      case "review":
        return w.buildReviewHTML ? w.buildReviewHTML(data) : "";
      case "android":
        return w.buildAndroidSMSHTML ? w.buildAndroidSMSHTML(data) : "";
      case "discord":
        return w.buildDiscordHTML ? w.buildDiscordHTML(data) : "";
      case "reddit":
        return w.buildRedditHTML ? w.buildRedditHTML(data) : "";
      case "bluesky":
        return w.buildBlueskyHTML ? w.buildBlueskyHTML(data) : "";
      case "newspaper":
        return w.buildNewspaperHTML ? w.buildNewspaperHTML(data) : "";
      case "forum":
        return w.buildForumHTML ? w.buildForumHTML(data) : "";
      case "facebook":
        return w.buildFacebookHTML ? w.buildFacebookHTML(data) : "";
      case "instagram":
        return w.buildInstagramHTML ? w.buildInstagramHTML(data) : "";
      case "sticky":
        return w.buildStickyNoteHTML ? w.buildStickyNoteHTML(data) : "";
      case "legal":
  ...
  var SKIN_EXIT_ZONE_PX = 44;
  function focusEditableAfterSkinBlock(editor, skinPos) {
  ...
  function skinActionsHTML() {
    return '<span class="skin-block-drag" data-drag-handle draggable="true" title="Drag to reorder" aria-label="Drag to reorder block">\u283F</span><button type="button" class="skin-block-action" data-skin-action="edit" aria-label="Edit block">Edit</button><button type="button" class="skin-block-action" data-skin-action="duplicate" aria-label="Duplicate block">Duplicate</button><button type="button" class="skin-block-action skin-block-action--delete" data-skin-action="delete" aria-label="Delete block">Delete</button>';
  }
  var SkinBlock = Node2.create({
    name: "skinBlock",
    group: "block",
    atom: true,
    // The text cursor treats this whole block as a single, unbreakable unit
    draggable: true,
    // Tells TipTap this node can be picked up and moved
    addAttributes() {
      return {
      ...
      return ({ node, getPos, editor }) => {
        ...
        actions.addEventListener("click", (e) => {
          ...
          if (action === "edit") {
            ...
            fire("ao3works-duplicate-skin", { getPos });
          } else if (action === "delete") {
            fire("ao3works-delete-skin", { getPos });
          }
        });
        return {
          dom,
  ...
  // src/editor.js
  var Ao3ToolbarShortcuts = Extension.create({
  ...
  function getAo3EditorExtensions() {
    return [
      StarterKit.configure({
        ...
        history: true,
        link: false,
        // We use the dedicated Link extension below
        underline: false
        // We use the dedicated Underline extension below
      }),
      Underline,
      TextAlign.configure({
      ...
      Image.configure({
        inline: false,
        allowBase64: true,
        // We allow it in the editor, but flag it for export
        HTMLAttributes: {
  ...
  var ALIGN_BLOCK_TAGS = /* @__PURE__ */ new Set(["P", "DIV", "H1", "H2", "H3", "H4", "H5", "H6"]);
  function parseTextDecoration(style2) {
    const m = (style2 || "").match(/text-decoration\s*:\s*([^;]+)/i);
    return m ? m[1].toLowerCase() : "";
  }
  function normalizePasteDecorations(doc3) {
    const candidates = Array.from(doc3.body.querySelectorAll("*")).reverse();
    for (const el of candidates) {
      const style2 = el.getAttribute("style") || "";
      if (!style2 || !/text-decoration/i.test(style2)) continue;
      const td = parseTextDecoration(style2);
      if (!td) continue;
      ...
      if (el.tagName === "U" && !hasStrike) continue;
      const parent = el.parentNode;
      if (!parent) continue;
      if (hasStrike && hasUnderline) {
        const u = doc3.createElement("u");
        const s = doc3.createElement("s");
  ...
  function extractBlockTextAlign(el) {
    const style2 = el.getAttribute("style") || "";
    let ta2 = "";
    const m = style2.match(/text-align\s*:\s*([^;]+)/i);
    if (m) ta2 = m[1].trim().toLowerCase();
    if (!ta2) {
      const al = (el.getAttribute("align") || "").toLowerCase();
      if (["left", "center", "right", "justify"].includes(al)) ta2 = al;
    }
    if (!ta2) {
      const cls = el.getAttribute("class") || "";
      const cm = cls.match(/align-(left|center|right|justify)\b/i);
      if (cm) ta2 = cm[1].toLowerCase();
    }
    if (["left", "center", "right", "justify"].includes(ta2)) return ta2;
    return "";
  }
  function sanitizePastedHTML(html2) {
    const doc3 = new DOMParser().parseFromString(html2, "text/html");
    let hasUnhostedImages = false;
    doc3.querySelectorAll("script, style, meta, link, xml").forEach((el) => el.remove());
    doc3.querySelectorAll("p, div, h1, h2, h3, h4, h5, h6").forEach((el) => {
      const style2 = el.getAttribute("style") || "";
      const marginLeft = style2.match(/(?:margin-left|padding-left)\s*:\s*([\d.]+)(px|pt|in|cm)/i);
      if (marginLeft) {
        ...
        if (px >= 20 && el.tagName !== "BLOCKQUOTE") {
          const bq = doc3.createElement("blockquote");
          while (el.firstChild) bq.appendChild(el.firstChild);
          el.replaceWith(bq);
        }
      }
    });
    normalizePasteDecorations(doc3);
    doc3.querySelectorAll("*").forEach((el) => {
      ...
      if (el.tagName === "IMG") {
        ...
        if (src.startsWith("data:image") || src.includes("googleusercontent.com") || src.startsWith("file://") || src.startsWith("blob:")) {
  ...
  var AUTOSAVE_KEY_BOOT = "aw.draft";
  ...
  function initEditor() {
    ...
    const editor = new Editor({
      ...
      editorProps: {
        ...
        transformPastedHTML(html2) {
          return sanitizePastedHTML(html2);
        },
        handlePaste(_view, event) {
          const clip = event.clipboardData;
          if (!clip) return false;
          const html2 = clip.getData("text/html") || clip.getData("text/plain") || "";
          const chapterImport = typeof window !== "undefined" ? window.ao3worksChapterImport : null;
          if (chapterImport && typeof chapterImport.shouldUseChapterImport === "function" && chapterImport.shouldUseChapterImport(html2)) {
            event.preventDefault();
